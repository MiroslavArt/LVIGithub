...
// стадия сделки
$stage = 'C10:NEW';
// перечисляем соответственно количеству товаров в корзине - PRODUCT_NAME (марка модель двигатель), количество, цена - за количество
$productarr = array(
    array('PRODUCT_ID' => 11556,
        'PRODUCT_NAME' => 'AUDI(FAW) A6L(488) 40TFSI',
        'QUANTITY' => 1,
        'PRICE' => 300),
    // 'PRODUCT_ID' всегда => 11556!
    array('PRODUCT_ID' => 11556,
        'PRODUCT_NAME' => 'CHRYSLER PT CRUISER(PT_) 1.6',
        'QUANTITY' => 2,
        'PRICE' => 300)
);
...
if($cid) {
    ...
    $out=curl_exec($curl);
    $out = json_decode($out, 1);
    $madeddeal = $out['result']['result']['make_deal'];
    $appParams = http_build_query(array(
        'halt' => 0,
        'cmd' => array(
            'get_contact' => 'crm.deal.productrows.set?'
                .http_build_query(array(
                    'id' => $madeddeal,
                    'rows' => $productarr
